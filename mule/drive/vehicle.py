...
from mule.drive import regulate
...
class Vehicle():
    ...
    def __init__(self):
        ...
        '''
        logger.info("Initializing vehicle")
    ...
    def add(self, part):
        ...
        logger.debug("Registering {} output key(s)".format(
            len(part.output_keys)))

        self.state_keys = self.state_keys.union(set(part.output_keys))

        logger.debug("Keys: {}".format(self.state_keys))
    ...
    def from_config(cls, config):
        ...
        '''
        logger.info('Loading parts from parsed config')

        vehicle = cls()

        for part in config:
            logger.debug("Adding {} {}".format(part.type, part.arguments))
    ...
    def start(self, config):
        ...
        for i, part in enumerate(self.parts):
            logging.info("Starting part {} of {}, {}".format(
                i + 1, len(self.parts), part))
            part.start()
    ...
    def drive(self, freq_hertz=10, verbose=False, verbosity=20):
        ...
        logger.info("Starting drive loop at {} Hz".format(freq_hertz))
        #if verbose == 'False':
        #    verbose = False
        #elif verbose == 'True':
        #    verbose = True
        ...
        try:
            for loop_nr in regulate(count(), freq_hertz):
                ...
                if verbose and loop_nr % verbosity == 0:
                    print("Loop", loop_nr, "Vehicle state variables: ",
                          self.print_state())
        ...
        #       and notify that this is expected behaviour
            ...
            logger.debug("KeyboardInterrupt".format())
            pass

    def stop(self):
        ''' Stops vehicle by stopping parts in reverse order '''
        for part in reversed(self.parts):
            logger.debug("Stopping {} ...".format(part._class_string))
            part.stop()
